from db_helper import get_treatment_recommendation
import torch
...
if __name__ == "__main__":
    model_path = "plant_disease_model.pth"  # Update with actual model path
    image_path = "test/test6.JPG"  # Update with actual test image path

    if not os.path.exists(image_path):
        print(f"Error: Image '{image_path}' not found!")
    else:
        model = load_model(model_path)
        predicted_disease = predict(image_path, model)
        print(f"Predicted Disease: {predicted_disease}")

        # Fetch treatment recommendation from database
        treatment_info = get_treatment_recommendation(predicted_disease)
